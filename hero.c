...
void afficher_hero(hero h, SDL_Surface *screen, background b)
{
	if (h.sprite.image != NULL)
	{
		...
		pos.x = h.position.x - b.position_background.x;
		pos.y = h.position.y - b.position_background.y;
...
void deplacer_hero(hero *h, background *b, int *Jcontinuer, character c)
{
	if (h->sprite.image != NULL)
	{
		...
		{
			if (b->position_background.x < b->image->w - SCREEN_WIDTH && b->position_background.x)
				b->position_background.x = (h->position.x + 5 * h->sprite.frame.w) - SCREEN_WIDTH;
			if (b->position_background.y < b->image->h - SCREEN_HEIGHT && b->position_background.y)
				b->position_background.y = (h->position.y + 3 * h->sprite.frame.h) - SCREEN_HEIGHT;
